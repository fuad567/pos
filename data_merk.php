...
<html lang="en">
  <head>
    ...
    <title>Data Merk Produk - Digital Talent</title>
  </head>
  <body style="background-color:#c2c2d6">
    <div class="container-fluid shadowb" style="padding:0px">
      <nav class="navbar navbar-expand-lg navbar-dark" style="background: linear-gradient(to right, #000099 0%, #7575a3 100%)!important">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <form class="form-inline my-0 my-lg-0" action="data_merk.php" method="get">
            <input class="form-control mr-sm-0" style="border-radius:0rem;background-color:#f0f0f5" name="cari" type="search" placeholder="Cari Data Merk" aria-label="Search">
    ...
    <div class="container" style="padding-bottom: 100px">
      ...
      <span style="padding-left:15px;padding-right:15px;padding-top:-5px;font-weight:600;font-size:18px">Data Merk Produk <span style="float:right"><a href="tambah_merk.php"><span class="btn btn-default btn-sm shadowb" style="border:1px solid white;background-color:#000080;color:white;margin-top:-1px;margin-right:10px">Tambah Merk</span></a></span></span>
      </div>
      ...
      <div class="row shadowb" style="padding-top:25px;padding-bottom: 10px;background-color:#7575a3;color:white">
        ...
        <tbody>
          ...
          if(!isset($_GET['cari'])) {
            $mahasiswa = mysqli_query($db, "SELECT * FROM merk");
          }
          else {
            $keyword = $_GET['cari'];
            $mahasiswa = mysqli_query($db, "SELECT * FROM merk WHERE nama_merk LIKE '%$keyword%'");
          }
        ...
        <tr>
          <th scope='row'>".$no."</th>
          <td>".$row['nama_merk']."</td>
          <td><a href='hapus_m.php?id=".$row['id_merk']."'><i class='fa fa-trash' style='color:white;' aria-hidden='true'></i></a> &nbsp;
              <a href='edit_merk.php?id=".$row['id_merk']."'><i class='fa fa-pencil' style='color:white' aria-hidden='true'></i></a></td>
        </tr>";
        $no++;
      } ?>
        </tbody>
      ...
      echo 'Total Merk Produk : <b>'. mysqli_num_rows($mahasiswa).'</b>';
    }
      ?></span>
      </div>
        </div>
