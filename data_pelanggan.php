...
<html lang="en">
  <head>
    ...
    <title>Data Pelanggan - Digital Talent</title>
  </head>
  <body style="background-color:#c2c2d6">
    ...
    <div class="container" style="padding-bottom: 100px">
      ...
      <span style="padding-left:15px;padding-right:15px;padding-top:-5px;font-weight:600;font-size:18px">Data Pelanggan <span style="float:right"><a href="tambah_pelanggan.php"><span class="btn btn-default btn-sm shadowb" style="border:1px solid white;background-color:#000080;color:white;margin-top:-1px;margin-right:10px">Tambah Pelanggan</span></a></span></span>
      </div>
      ...
      <div class="row shadowb" style="padding-top:25px;padding-bottom: 10px;background-color:#7575a3;color:white">
        ...
        <tbody>
          ...
          if(!isset($_GET['cari'])) {
            $mahasiswa = mysqli_query($db, "SELECT * FROM pelanggan");
          }
          else {
            $keyword = $_GET['cari'];
            $mahasiswa = mysqli_query($db, "SELECT * FROM pelanggan WHERE id_pelanggan LIKE '%$keyword%' OR nama_pelanggan LIKE '%$keyword%' OR alamat LIKE '%$keyword%' OR jenis_kelamin LIKE '%$keyword%' OR username LIKE '%$keyword%'");
          }
        ...
        <tr>
          ...
          <td>".$row['username']."</td>
          <td>".$row['alamat']."</td>
          <td><a href='hapus_p.php?id=".$row['id_pelanggan']."'><i class='fa fa-trash' style='color:white;' aria-hidden='true'></i></a> &nbsp;
              <a href='edit_pelanggan.php?id=".$row['id_pelanggan']."'><i class='fa fa-pencil' style='color:white' aria-hidden='true'></i></a></td>
        </tr>";
        $no++;
      } ?>
        </tbody>
      ...
      echo 'Total Pelanggan : <b>'. mysqli_num_rows($mahasiswa).'</b>';
    }
      ?></span>
      </div>
        </div>
