...
<html lang="en">
  <head>
    ...
    <title>Data Transaksi - Digital Talent</title>
  </head>
  <body style="background-color:#c2c2d6">
    ...
    <div class="container" style="padding-bottom: 100px">
	  ...
	  <span style="padding-left:15px;padding-right:15px;padding-top:-5px;font-weight:600;font-size:18px">Data Transaksi <span style="float:right"><a href="tambah_transaksi.php"><span class="btn btn-default btn-sm shadowb" style="border:1px solid white;background-color:#000080;color:white;margin-top:-1px;margin-right:10px">Tambah Transaksi</span></a></span></span>
	  </div>
      ...
      <div class="row shadowb" style="padding-top:25px;padding-bottom: 10px;background-color:#7575a3;color:white">
            ...
            <table class="table" style="color:white">
			  ...
			  <tbody>
				  <?php
					...
					if(!isset($_GET['cari'])) {
					  // WORK $mahasiswa = mysqli_query($db,"SELECT * FROM produk INNER JOIN merk ON produk.id_MERK=merk.id_merk INNER JOIN kategori_produk on produk.id_kategori=kategori_produk.id_kategori;");
					  $mahasiswa = mysqli_query($db, "SELECT * from transaksi t
						INNER JOIN pelanggan p on t.id_pelanggan = p.id_pelanggan 
						INNER JOIN produk pr on t.id_produk = pr.id_produk
						LEFT JOIN merk m on pr.id_merk = m.id_merk");
					}
					else {
					  $keyword = $_GET['cari'];
				...
				<tr>
				  ...
				  <td>".$row['nama_merk']."</td>
				  <td>".$row['total_transaksi'] / $row['harga']."</td>
				  <td>".$row['total_transaksi']."</td>
				  <td><a href='hapus_t.php?id=".$row['id_transaksi']."'><i class='fa fa-trash' style='color:white;' aria-hidden='true'></i></a></td>
				</tr>";
				$no++;
			} ?>
			  </tbody>
        ...
        if(mysqli_num_rows($mahasiswa) <= 0) {
          echo '<p style="text-align:center">Data tidak ditemukan</p>';
			...
			echo 'Total Transaksi : <b>'. mysqli_num_rows($mahasiswa).'</b>';
		}
      ?></span>
			</div>
        </div>
